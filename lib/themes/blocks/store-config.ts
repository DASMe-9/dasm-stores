...
/**
 * Read the block document, migrating older shapes transparently:
 * - v1: `{ version, source }`  -> landing surface + default products + default design
 * - v2: `{ version, surfaces }` -> + default design backfilled
 * - v3: `{ version, surfaces, design }`
 */
export function readBlockDocument(themeConfig: Record<string, unknown> | null | undefined): BlockDocument {
  ...
  if (raw && typeof raw === "object") {
    ...
    const design = validateDesign(doc.design);
    // v2/v3: explicit surfaces
    if (doc.surfaces && typeof doc.surfaces === "object") {
      ...
      return { version: BLOCK_EDITOR_VERSION, surfaces: { landing, products }, design };
    }
    ...
    if (typeof doc.source === "string" && doc.source.trim()) {
      return {
        ...
        surfaces: { landing: doc.source, products: DEFAULT_PRODUCTS_SOURCE },
        design,
      };
    }
  }
