      ...
      <div className="border-b border-[var(--border)] bg-[var(--card)] px-3 py-2.5">
        ...
        <div className="mt-2.5 flex gap-1.5 overflow-hidden">
          {previewNav.map((item) => (
            <span
              key={item}
              className="rounded-full border border-[var(--border)] bg-[var(--muted)] px-2 py-1 text-[9px] font-bold text-[var(--muted-foreground)]"
            >
              {item}
            </span>
          ))}
      ...
      <div className="grid grid-cols-3 gap-2 p-3">
        {previewProducts.map((name, index) => (
          <article key={name} className={cardClass}>
            <div
              className="store-product-card__media relative flex aspect-[4/5] items-center justify-center overflow-hidden bg-[var(--muted)]"
              style={{
                background:
                  index === 0
            ...
            >
              <span className="absolute inset-x-3 top-3 h-1.5 rounded-full bg-white/28" />
              <span className="absolute bottom-3 right-3 h-8 w-8 rounded-xl bg-white/18 backdrop-blur" />
              {index === 1 ? (
                <Sparkles className="h-6 w-6 text-white/90" aria-hidden />
              ) : (
                <ShoppingBag className="h-6 w-6 text-white/90" aria-hidden />
              )}
            </div>
            <div className="store-product-card__body space-y-1.5 p-2">
              <p className="truncate text-[10px] font-bold text-[var(--foreground)]">
                {name}
              </p>
