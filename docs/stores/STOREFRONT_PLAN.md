...
- **Auth موحّد عبر Core** — `dasm-stores` يملك صفحات دخول/تسجيل/تحقق محلية للمنتج، لكنها تستدعي Core API فقط ولا تنشئ هوية أو مستخدمين داخل DASM-services.
- **تسجيل المتاجر = صاحب متجر فقط** — لا تعرض الواجهة اختيارات `user` / `dealer` / `venue_owner` / `investor`; Core يربط الهوية ويحتفظ بالأدوار، وDASM-services يحتفظ ببيانات المتجر التشغيلية.
- **لا إنشاء تلقائي للمتجر** — صاحب المعرض أو أي مستخدم لا يحصل على متجر تلقائيا؛ إنشاء المتجر يتم بطلب صريح عبر Core `/api/stores/my-store`.
