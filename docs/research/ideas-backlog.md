...
| 2026-07-05 | competitors/2026-30 | **Product Disclosures block (صفحة المنتج):** Shopify Dawn 15.5.0 أضافت قسمًا لعرض نصوص الإفصاح القانوني/التنظيمي على صفحة المنتج (مكوّنات، تحذيرات، ترخيص). مهم للمتاجر السعودية في الغذاء/التجميل/المكملات. `app/[slug]/products/[productId]/page.tsx` يعرض `product.description` فقط. يتوقف على وجود حقل `disclosures` أو `regulatory_info` في product API. | `app/[slug]/products/[productId]/page.tsx` | 🟡 متوسطة | مؤجلة — تحقق API مطلوب |
