...
| 2026-08-03 | competitors/2026-31 | **Wishlist للضيف (guest wishlist):** Salla أطلقت guest wishlist في `salla-product-card` — يُخزَّن في localStorage ويُرحَّل عند تسجيل الدخول. الـ specs المعلقة (`product-tile-wishlist-2026-06-11.md` + `product-card-store-wishlist-2026-06-12.md`) لا تتضمن حالة الضيف. يجب تحديثهما أو إنشاء spec يشمل: `localStorage` لحفظ IDs، مزامنة عند تسجيل الدخول، وReset عند تسجيل الخروج. | `components/product/ProductCard.tsx` + `app/page.tsx` (ProductTile) + context/hook جديد | 🟡 متوسطة | مؤجلة — يُعدَّل على specs المعلقة في sprint قادم |
