...
| 2026-07-14 | competitors/2026-30 | **توصيات منتجات بالذكاء الاصطناعي:** Salla Twilight 2.14.490 أضافت AI product recommendation APIs. صف "قد يعجبك أيضاً" على صفحة تفصيل المنتج. يتوقف على دعم user context في `getExploreProducts()` أو endpoint منفصل من الـ backend. | `app/[slug]/products/[productId]/page.tsx` | 🔴 عالية | مؤجلة — تحتاج تحقق API + backend feature |
| 2026-07-14 | competitors/2026-30 | **تقييمات متعددة الأبعاد مع صور المشترين:** Salla Twilight 2.14.490 دعمت مراجعات تشمل: أبعاد متعددة (جودة/توصيل/مطابقة)، صور المشتري، تعديل/حذف المراجعة. `ProductReviews.tsx` موجود لكنه أساسي. تحتاج `reviews[].images` و`reviews[].dimensions` في الـ backend قبل البناء. | `components/product/ProductReviews.tsx` | 🟡 متوسطة | مؤجلة — تحتاج تحقق API + backend feature |
