...
| 2026-06-16 | competitors/2026-29 | **معلومات شحن/استلام على صفحة تفصيل المنتج:** Salla أضافت مكوّن `salla-fulfillment-methods` يعرض خيارات توصيل/استلام قبل زر "أضف للسلة". dasm-stores لا يعرض أي معلومة شحن على صفحة المنتج. يتوقف على حقول API: يحتاج تحقق من بيانات متاحة في `getProducts()` response. | `app/[slug]/products/[productId]/page.tsx` | 🟡 متوسطة | **تحوّل إلى spec** `specs/product-delivery-info-strip-2026-07-23.md` — API متاحة عبر `store.shipping_configs` |
| 2026-07-23 | competitors/2026-30 | **منتجات مشابهة (RelatedProducts) على صفحة المنتج:** `app/[slug]/products/[productId]/page.tsx` لا يعرض أي توصيات أو منتجات مشابهة بعد وصف المنتج. Salla (`product.similar-products`)، Zid، وShopify Dawn يعرضون 4-6 منتجات مقترحة أسفل الصفحة. يرفع معدل التحويل ومتوسط قيمة الطلب. يحتاج API call إضافي: `getProducts(slug, {sort: 'featured', per_page: 4})` — بيانات متاحة. | `app/[slug]/products/[productId]/page.tsx` + `components/product/RelatedProducts.tsx` جديد | 🟡 متوسطة | مؤجلة |
