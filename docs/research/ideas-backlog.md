...
| 2026-08-14 | competitors/2026-33 | **توصيات منتجات بالذكاء الاصطناعي على الصفحة الرئيسية:** Salla Twilight 2.14.490 دعمت recommendation API لعرض "اقتراحات مخصصة لك" بناءً على سلوك المتسوق. dasm-stores تعرض 12 منتجاً بـ `sort: "featured"` فقط. يتوقف تطبيقه على recommendation endpoint في `lib/api-server.ts`. | `app/page.tsx` — قسم جديد | 🟢 مرتفعة | مؤجلة — يتوقف على API |
| 2026-08-14 | competitors/2026-33 | **قسم التقييمات مع رفع صور (مرحلة لاحقة):** Salla 2.14.490 أضافت تقييمات بمحاور متعددة (جودة/تغليف/توصيل) + رفع صور عند التقييم. dasm-stores لا تعرض مراجعات حالياً. الفكرة تحتاج backend reviews API أولاً. | `app/[slug]/products/[productId]/page.tsx` + API reviews endpoint | 🔵 منخفضة — مرحلة لاحقة | مؤجلة |
