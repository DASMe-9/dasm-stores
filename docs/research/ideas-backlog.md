...
| 2026-06-16 | competitors/2026-29 | **معلومات شحن/استلام على صفحة تفصيل المنتج:** Salla أضافت مكوّن `salla-fulfillment-methods` يعرض خيارات توصيل/استلام قبل زر "أضف للسلة". dasm-stores لا يعرض أي معلومة شحن على صفحة المنتج. يتوقف على حقول API: يحتاج تحقق من بيانات متاحة في `getProducts()` response. | `app/[slug]/products/[productId]/page.tsx` | 🟡 متوسطة | **تحوّلت لـ spec** — `product-info-accordion-2026-08-11.md` (بيانات `fulfillment_policy` متاحة) |
| 2026-08-11 | competitors/2026-32 | **تتبع موضع المنتج في أحداث Analytics:** Salla 2.14.535 أضافت `position` ضمن أحداث `product_view` و`add_to_cart` في قوائم المنتجات. dasm-stores لا يُرسل موضع المنتج في أي analytics event. التطبيق يتطلب طبقة analytics مستقلة عن CSS tokens. | `app/page.tsx` (ProductTile) + طبقة analytics | 🔵 منخفضة | مؤجلة — يتطلب قرار analytics architecture |
